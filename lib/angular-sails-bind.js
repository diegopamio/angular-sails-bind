    ...
    function ($q, $rootScope, $timeout, $log, $sails) {
        ...
        function fireEvent(eventName, options){
            if(angular.isString(eventName)){
                ...
                var broadcastType = getConfigurationOption('broadcastType', options).toLowerCase();
                if(eventRegister.hasOwnProperty(eventName)){
        ...
        function createEventObject(options, item, verb, data){
            if(angular.isString(item)){
                var eventObj = {verb: item, scope: options.scope.$id};
                data = verb;
            }else{
                var eventObj = {id: item.id, verb: verb, scope: options.scope.$id};
            }
